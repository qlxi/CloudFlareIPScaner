...
const (
    ...
    batchSize   = 10000 // Размер партии IP-адресов для обработки
)
...
func main() {
    ...
    // If no command-line arguments, start the GUI
    if flag.NFlag() == 0 {
        createGUI()
        return
    }

    // Otherwise, run the scan with command-line arguments
    runScan(*asnList, *defaultPort, *maxThreads, *enableTLS)
}

func createGUI() {
    myApp := app.New()
    myApp.Settings().SetTheme(theme.DarkTheme())
    myWindow := myApp.NewWindow("CloudFlare IP Scanner By nxhack")
    myWindow.Resize(fyne.NewSize(400, 300))

    asnEntry := widget.NewEntry()
    asnEntry.SetPlaceHolder("Введите номера ASN, разделенные запятыми")

    portEntry := widget.NewEntry()
    portEntry.SetText("443")

    maxThreadsEntry := widget.NewEntry()
    maxThreadsEntry.SetText("50")

    enableTLSCheck := widget.NewCheck("Включить TLS", func(checked bool) {
        *enableTLS = checked
    })
    enableTLSCheck.SetChecked(true) // Ensure the checkbox is checked by default

    startButton := widget.NewButtonWithIcon("Начать сканирование", theme.ConfirmIcon(), func() {
        asn := asnEntry.Text
        port, err := strconv.Atoi(portEntry.Text)
        if err != nil {
            log.Println("Ошибка порта:", err)
            return
        }
        maxThreadsVal, err := strconv.Atoi(maxThreadsEntry.Text)
        if err != nil {
            log.Println("Ошибка макс. потоков:", err)
            return
        }
        tls := enableTLSCheck.Checked

        go runScan(asn, port, maxThreadsVal, tls)
    })

    grid := container.NewGridWithRows(7,
        widget.NewLabelWithStyle("Введите параметры для сканирования ASN:", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
        asnEntry,
        widget.NewLabelWithStyle("Порт:", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
        portEntry,
        widget.NewLabelWithStyle("Максимальное количество потоков:", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
        maxThreadsEntry,
        enableTLSCheck,
    )
    ...
    contentWithPadding := container.NewCenter(container.NewVBox(
        widget.NewLabelWithStyle("CloudFlare IP Scanner", fyne.TextAlignCenter, fyne.TextStyle{Bold: true, Italic: true}),
        content,
    ))

    myWindow.SetContent(contentWithPadding)
    myWindow.ShowAndRun()
}

func runScan(asn string, port, maxThreadsVal int, tls bool) {
    *asnList = asn
    *defaultPort = port
    *maxThreads = maxThreadsVal
    *enableTLS = tls
...
func mainScan() {
    ...
    for _, asn := range asns {
        ...
        clearConsole() // Очистка консоли перед началом нового сканирования ASN
        startTime := time.Now()
