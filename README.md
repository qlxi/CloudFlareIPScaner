...
CloudFlare IP Scanner - это программа для сканирования IP-адресов ASN с использованием интерфейса командной строки и графического интерфейса. Программа позволяет определить местоположение CloudFlare IP CDN и измерить задержку (latency) IP-адресов.
...
Пример:
    ...
    ```sh
    ./CloudFlareIPScanner -asn "13335,15169" -port 443 -max 50 -tls true
    ```
